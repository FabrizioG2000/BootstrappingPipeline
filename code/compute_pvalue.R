...
feature_annotation_fn <- function(txdb, chr_feature_Grange, fn_file) {
  ...
  # Annoates the features using the database
...
#-----------------------------------------
cluster_folder <- "./data/cluster/wrapped/"
cluster_file_suffix <- "_wrapped.Rda"
feature_file <- "./data/feature/feature_wrapped.Rda"
out_file <- "./data/pvalue_table.Rda"

# Loading the Genomic Range Objects.
wrapped_features <- get(load(feature_file)) # The GRange object
...
# The repository containing the annotation data.
annotation_repo <- "./data/annotation/fn_BED/"

txdb <- TxDb.Hsapiens.UCSC.hg19.knownGene

chr_set <- list.files(annotation_repo)
cl_chr_emp_pval_l <- lapply(chr_set, function(chromo) {
  cl_chr_tbl <- empirical_pval_compute_fn(chromo, cluster_folder, cluster_file_suffix, wrapped_features, annotation_repo, txdb, hg19_coord)
